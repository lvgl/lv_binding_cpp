...
LvObj& LvObj::setParent(lv_obj_t *parent) {
	lv_obj_set_parent(cObj.get(), parent);
	return *this;
}
...
const lv_coord_t* LvObj::getStyleGridColumnDscArray(
		uint32_t part) const noexcept {
...
struct _lv_event_dsc_t* LvObj::addEventCb(lv_event_cb_t event_cb,
...
LvObj& LvObj::alignTo(const lv_obj_t *base, lv_align_t align, lv_coord_t x_ofs,
		lv_coord_t y_ofs) {
...
LvObj& LvObj::moveChildrenBy(lv_coord_t x_diff, lv_coord_t y_diff,
...
LvObj& LvObj::addStyle(lv_style_t *style, lv_style_selector_t selector) {
	lv_obj_add_style(cObj.get(), style, selector);
	return *this;
}
LvObj& LvObj::removeStyle(lv_style_t *style, lv_style_selector_t selector) {
	lv_obj_remove_style(cObj.get(), style, selector);
	return *this;
}
...
lv_style_value_t LvObj::getStyleProp(lv_part_t part,
		lv_style_prop_t prop) const noexcept {
	return lv_obj_get_style_prop(cObj.get(), part, prop);

}
LvObj& LvObj::setLocalStyleProp(lv_style_prop_t prop, lv_style_value_t value,
		lv_style_selector_t selector) {
	lv_obj_set_local_style_prop(cObj.get(), prop, value, selector);
	return *this;
}
lv_res_t LvObj::getLocalStyleProp(lv_style_prop_t prop, lv_style_value_t *value,
		lv_style_selector_t selector) const noexcept {
	return lv_obj_get_local_style_prop(cObj.get(), prop, value, selector);

}
bool LvObj::removeLocalStyleProp(lv_style_prop_t prop,
		lv_style_selector_t selector) {
	return lv_obj_remove_local_style_prop(cObj.get(), prop, selector);

}
LvObj& LvObj::styleCreateTransition(lv_part_t part, lv_state_t prev_state,
		lv_state_t new_state, const _lv_obj_style_transition_dsc_t *tr_dsc) {
	_lv_obj_style_create_transition(cObj.get(), part, prev_state, new_state,
			tr_dsc);
	return *this;
}
_lv_style_state_cmp_t LvObj::styleStateCompare(lv_state_t state1,
		lv_state_t state2) {
